...
#iev analysis
library(iev)
ievsamples<-read.csv(file.choose())  # choose the data
startievtime<-Sys.time()  # start timing
necessaryconditions(0.9,0.5,samples=ievsamples)  # necessaryconditionsanalysis
iev(k=5,consistencythreshold=0.8,samples=ievsamples)  # iev analysis
...
print(paste("The total time of iev analysis is:",extractanalysistime))  # print the analysis time

#MDR analysis
library(MDR)
samplesresult<-list()  # define an empty list
samplesresult<-c(samplesresult,ievsamples[[length(ievsamples)]])  # extract the result column
...
for (i in 1:(length(ievsamples)-1)){  # put the other columns
condition<-list()
condition<-c(condition,ievsamples[[i]])
...
ch<-chisquare(remaxcombinationlist,ievsamples)  # chi-square test，remaxcombinationlist is the pattern，ievsamples is the source dataset
